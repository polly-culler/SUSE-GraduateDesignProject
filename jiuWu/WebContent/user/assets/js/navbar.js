...
Q6MenuBar.prototype.toString = function() {
	// menuBar对应的<div>
	var str = '<ul  class = "emeriss-clone-mobile-menu emeriss-nav main-menu " id="menu-main-menu " name="menuBar" onClick="' + this.obj + '.showMenu(event, this)">\n';
	// menuBar的标题<div>
...
Q6MenuBar.prototype.menu2Str = function(menuName){
	...
	// 菜单对应的<div>
	var str = '<li name="menu" class="menu-item menu-item-has-children ">\n';
	// 菜单标题对应的<div>，其中包含icon与text两部分。
	str += '<a name="menuTitle" class="emeriss-menu-item-title ">' + menuName + '<span class="toggle-submenu"></span></a>\n';
	// 菜单内容对象的<div>，所有菜单荐都存放在这个<div>中。
	str += '<ul class="submenu"  name="menuContent">\n';
...
Q6MenuBar.prototype.item2Str = function(menuItem){
	return '<li class="menu-item" onMouseMove="' + this.obj + '.itemMouseMove(this)" onMouseOut="' + this.obj + '.itemMouseOut(this)" onClick="skip(\'' + menuItem.url + '\', \'' + menuItem.frameName + '\')"><a href="#">' + menuItem.menuItemName + '</a></li>\n';
}
...
//		alert(tagName + ", " + name);
		...
		tagName = res.tagName;
		name = res.getAttribute("name");
		if(tagName == "UL" && name == "menuContent") {
			return null;
		}
	}
	return res;
